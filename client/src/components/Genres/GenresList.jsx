...
function GenresList() {
  ...
  const {
    ...
    error,
    refetch,
  } = usePagination(`/genres`, itemsPerPage, currentPage);
      ...
      <Box sx={scrollListBoxStyle}>
        <List>
          {loading
            ...
            : genres.map((genre) => (
                <Stack key={genre.uuid} direction='column' marginBottom={1}>
                  <ListItem
                    disablePadding
                    component={Link}
                    sx={itemListStyle}
                    to={`/genres/${genre.uuid}`}
                  >
                    <ListItemButton sx={styleListListItemButton}>
                      <ListItemAvatar>
                        <StyledAvatar src={genre.logo} />
                      </ListItemAvatar>
                      <ListItemText
                        primary={`${genre.title || 'Unknown genre'}`}
                      />
                    </ListItemButton>
